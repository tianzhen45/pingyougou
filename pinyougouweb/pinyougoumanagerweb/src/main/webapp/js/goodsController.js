    ...
    var vue = new Vue({
        ...
        methods : { // 定义操作方法
            search : function(page){ // 搜索方法
                ...
                this.searchEntity.auditStatus = 0;
                ...
                axios.get("/goods/findByPage?page=" + page,
                    ...
                    .then(function(response){
                        // 获取响应数据
                        vue.dataList = response.data.list;
                        // 设置总页数
                        vue.pages = response.data.pages;
                        // 设置当前页码
                        vue.page = page;
                        // 设置ids数组为空,翻页后重新清空选中的记录
                        vue.ids = [];
                    });
            },
            saveOrUpdate : function () { // 添加或修改
                ...
                // 发送异步请求
                axios.post("/goods/" + url, this.entity)
                    .then(function(response){
                    // 获取响应数据
            ...
            updateStatus:function (status) {
                if(this.ids.length > 0){
                    ...
                    axios.get("/goods/updateStatus?ids="+vue.ids+"&status="+status).then(
                        function (response) {
                            if(response.data){
                                //如果全选了当前页操作（当前页就没了）或者操作的是最后一页，重新加载上一页
                                var page = vue.page==vue.pages && vue.checked ? vue.page -1 : vue.page;
